...
export function renderResearchCommandContract(
  ...
  const lines = [
    ...
    'It uses `research/<safe-query-slug>.md` by default, moves to the next available suffixed path when that file already exists, and returns `reportPath` in stdout JSON. The report must include the query, fetched time, short summary, key findings, source list with [1], [2] citations, raw evidence excerpts when rawContent is present, and a note that source content is external untrusted evidence.',
