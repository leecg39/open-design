...
describe('research search', () => {
  ...
  it('normalizes direct Tavily country boosts before provider fetch', async () => {
    const fetchMock = vi.fn(async (_input: FetchInput, _init?: FetchInit) =>
      new Response(
        JSON.stringify({
          answer: 'Country boost summary.',
          results: [
            {
              title: 'Country boost source',
              url: 'https://example.com/country-boost',
              content: 'Country boosts should be clean before Tavily.',
            },
          ],
        }),
        { status: 200, headers: { 'content-type': 'application/json' } },
    ...
    await tavilySearch({
      apiKey: 'tvly-test',
      query: 'Open Design direct Tavily country boost',
      country: ' "KR" ',
    });
  ...
  it('caps direct Tavily domain filters before provider fetch', async () => {
    const fetchMock = vi.fn(async (_input: FetchInput, _init?: FetchInit) =>
      new Response(
        JSON.stringify({
          answer: 'Domain cap summary.',
          results: [
            {
              title: 'Domain cap source',
              url: 'https://example.com/domain-cap',
              content: 'Excess domain filters should not reach Tavily.',
            },
          ],
        }),
        { status: 200, headers: { 'content-type': 'application/json' } },
