...
describe('ChatComposer /search command', () => {
  ...
  it('keeps equals-style multi-word /search country values out of the query', () => {
    const onSend = vi.fn();
    ...
    expect(prompt).toContain('Canonical query:\n\n```text\nAI design market\n```');
    expect(prompt).not.toContain('Korea" AI design market');
    expect(meta).toEqual({
      research: {
        enabled: true,
        query: 'AI design market',
  ...
  it('warns when /search country values are outside Tavily country options', () => {
    const onSend = vi.fn();
    ...
    fireEvent.change(screen.getByTestId('chat-composer-input'), {
      target: { value: '/search --country Atlantis Open Design market' },
    });
    fireEvent.click(screen.getByTestId('chat-send'));
    ...
    expect(prompt).toContain('Research parser warning: Ignored invalid --country value.');
    expect(meta.research?.country).toBeUndefined();
  });
  ...
  it('deduplicates /search domains across repeated flags', () => {
    ...
    fireEvent.change(screen.getByTestId('chat-composer-input'), {
      target: {
        value:
          '/search --include-domains openai.com --include-domains openai.com,docs.openai.com OpenAI platform releases',
      },
    });
    fireEvent.click(screen.getByTestId('chat-send'));
    ...
    const [prompt, _attachments, _commentAttachments, meta] = onSend.mock.calls[0]!;
    expect(prompt).toContain(
      '--depth shallow --include-domains openai.com,docs.openai.com --max-sources 5',
    );
    expect(prompt).toContain(
      'Research parser warning: Removed duplicate --include-domains entries.',
    );
    expect(meta).toEqual({
      research: {
        enabled: true,
